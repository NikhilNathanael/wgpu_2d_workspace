...
use quasiquote::{quote::quote, quasiquote};
use syn::{parse, DataStruct, DeriveInput, Error, Fields, Ident, Index, Type};
...
pub fn vertex_buffer_data (data: TokenStream) -> TokenStream {
	...
	let strct: DataStruct = match strct.data {
		...
		syn::Data::Enum(x) => return Error::new(x.enum_token.span, "Vertex Buffer Data cannot be used on enums").to_compile_error().into(),
		syn::Data::Union(x) => return Error::new(x.union_token.span, "Vertex Buffer Data cannot be used on unions").to_compile_error().into(),
	};
	...
	let fields: Vec<(Ident, Type)> = {
		match strct.fields {
			...
			Fields::Unit => return Error::new(strct.struct_token.span, "Vertex Buffer Data cannot be used on unit structs").to_compile_error().into(),
		}
	};

	let wgpu_buffer_path = fields.iter().map(|_| quote!(crate::wgpu_context::WGPUBuffer)).collect::<Vec<_>>();
...
pub fn uniform_buffer_data (data: TokenStream) -> TokenStream {
	...
	match strct.data {
		syn::Data::Struct(_) => (),
		syn::Data::Enum(x) => return Error::new(x.enum_token.span, "Uniform Buffer Data cannot be used on enums").to_compile_error().into(),
		syn::Data::Union(x) => return Error::new(x.union_token.span, "Uniform Buffer Data cannot be used on unions").to_compile_error().into(),
	}
